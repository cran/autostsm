...
The second step is wavelet analysis for seasonal detection, which is performed by regressing a series of sin-cos waves (Fourier series) on the detrended data. The detrended data may also be rescaled by the trend if the Cox-Stuart dispersion tests suggests that the detrended data does not have a constant amplitude. The regression is performed sequentially, starting with a wave of periodicity equal to that of the data's frequency and working back towards a wave with periodicity 2, using the equation

$$
\tilde{y_t} = \sum_{i = 1}^j (a_j sin(j) + b_j cos(j))
$$
Selection is performed using forward stepwise regression for seasonal frequencies of minutely, hourly, daily, weekend, weekday, weekly, monthly, quarterly, semi-yearly, and yearly where applicable for a more parsimonious model and to reduce the likelihood of spurious seasonalities from being detected. Alternatively, the user can specify `full_spectrum = TRUE` to check every seasonal length that has at least 3 cycles observed in the data. After each iteration, a $\chi^2$ test is performed for the sum of each pair of coefficients, $\chi_j^2 = a_j^2 + b_j^2$, which corresponds to the squared power of each frequency. Any frequency that is not statistically significant is discarded before the next iteration so that only statistically significant frequencies are kept at each iteration. After all iterations, the remaining frequencies are selected as those that describe the seasonal pattern in the data. The statistical significance level used is fixed at 0.0001 (or 0.01%). This level is set to prevent spurious seasonal frequencies from being included in the model. 


### Cycle Detection 

Next, wavelet analysis for cycle detection is performed by regressing a series of sin-cos waves (Fourier series) on the detrended data. This new series may also be rescaled by the trend if the Cox-Stuart dispersion tests suggests that the detrended data does not have a constant amplitude. The regression is performed individually for each wave using the equation
