...
stsm_init_vals = function(y, par, freq, trend, decomp = "", seasons = NULL, prior = NULL, cycle = NULL){
  ...
  if("trend" %in% colnames(prior)){
    init[["B0"]][rownames(init[["B0"]]) == "Tt_0", ] = prior[!is.na(trend), ]$trend[1]
  }
  if("cycle" %in% colnames(prior)){
    init[["B0"]][rownames(init[["B0"]]) %in% c("Ct_0", "Cts_0"), ] = prior[!is.na(cycle), ]$cycle[1]
  }
  if("drift" %in% colnames(prior)){
    init[["B0"]][rownames(init[["B0"]]) == "Dt_0", ] = prior[!is.na(drift), ]$drift[1]
  }
  if("remainder" %in% colnames(prior)){
    init[["B0"]][rownames(init[["B0"]]) == "et_0", ] = prior[!is.na(remainder), ]$remainder[1]
  }
  if(grepl("seasonal", decomp)){
    if(!is.null(seasons)){
      for(j in seasons){
        init[["B0"]][rownames(init[["B0"]]) %in% paste0(c("St", "Sts"), j, "_0"), ] = 
